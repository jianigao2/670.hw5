...
;; Implementing: sheep are scared of wolves.
...
to-report are-there-sheep-near-me
  let sheep-near-me
  (list
    (count sheep-on patch-at-heading-and-distance 0 1)
    (count sheep-on patch-at-heading-and-distance -90 1)
    (count sheep-on patch-at-heading-and-distance -180 1)
    (count sheep-on patch-at-heading-and-distance 90 1)
  )
  report sheep-near-me
end

to move-wolves
  ;;; STEP ONE: where are the sheep near by?
  let near-sheep are-there-sheep-near-me

  ifelse ( wolves-chase-sheep)
  [
    ifelse ( max near-sheep > 0 )
    [ifelse (item 0 near-sheep = max near-sheep)
      [ ;; they're in front of me! chase!
        fd wolf-speed]
      [ ;; they are not in front of me. then where?
        ifelse (item 1 near-sheep = max near-sheep)
          [lt wolf-speed]
        [
          ifelse (item 2 near-sheep = max near-sheep)
          [ bk wolf-speed]
          [rt wolf-speed]
        ]
       ]
      ]
    [ ;; there are no sheep near me, act normally.
      rt random 50
      lt random 50
      fd 1
    ]
  ]
  [ ;; wolves don't chase sheep, act normally
    rt random 50
    lt random 50
    fd 1
  ]
...
ifelse (sheep-reproduce-energy) [
if energy >   sheep-gain-from-food[
set energy (energy / 2 )
hatch 1 [ rt random-float 360 fd 1]
]
]
[
  ifelse count sheep < 360[
...
to eat-sheep  ; wolf procedure
  ...
  if prey != nobody  [                          ; did we get one?  if so,
    ifelse wolves-gain-sheep-energy
    [
      print (sentence prey )
      ask prey [ show energy ]
      ;;    set energy energy + energy of prey ;; get the energy of the sheep
...
to-report wolves-chase-sheep-plotter
  let max-val initial-number-sheep + initial-number-wolves
  ifelse wolves-chase-sheep [ report max-val ] [ report 0]
end
...
"sheep-scared" 1.0 0 -1664597 true "" "plot sheep-scared-of-wolves-plotter"
